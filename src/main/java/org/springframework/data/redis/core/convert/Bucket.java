...
public class Bucket {
    ...
    public void resetDate(String filedName) {
        String s = new String(this.data.get(filedName),CHARSET);
        Map<String,String> map = JSON.parseObject(s,Map.class);
        this.data.clear();
        map.forEach((k,v) -> {
            if (!ObjectUtils.isEmpty(v)) {
                try {
                    this.data.put(k,v.getBytes("UTF-8"));
    ...
    public Map<byte[], byte[]> rawMap() {
        ...
        if (data.containsKey(pro)) {
            raw.put(pro.getBytes(CHARSET),data.get(pro));
            data.remove(pro);
        }
       /* Jackson2JsonRedisSerializer serializer = new Jackson2JsonRedisSerializer();
        serializer.setObjectMapper(new ObjectMapper());*/

        Map<String, String> collect = data.entrySet().stream()
        ...
        System.out.println("+++++++++++++++++++++++++++++++++转object->json+++++++++++++++++++++++++++++++++++++++");
        raw.put("objectValue".getBytes(CHARSET), JSON.toJSONString(collect).getBytes(CHARSET));
